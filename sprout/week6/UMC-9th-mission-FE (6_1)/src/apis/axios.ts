...
import { LOCAL_STORAGE_KEY } from '../constants/key'; // L2: ../ 추가, .ts 제거
import { useLocalStorage } from '../hooks/useLocalStorage'; // L3: ../ 추가, .ts 제거
...
axiosInstance.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    // 401 에러이면서, 아직 재시도 하지 않은 요청 처리
    if (
      error.response &&
      error.response.status === 401 &&
      !originalRequest._retry
    ) {
      // refresh 엔드포인트 401 에러가 발생한 경우 (Unauthorized), 중복 재시도를 방지하고 로그아웃 처리
