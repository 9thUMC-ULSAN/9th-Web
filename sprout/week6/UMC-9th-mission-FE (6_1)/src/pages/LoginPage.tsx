...
const LoginPage = (): ReactElement => {
  ...
  // ✅ [수정 1] Google 로그인 처리 함수를 컴포넌트 본문에 독립적으로 정의
  const handleGoogleLogin = () => {
    // VITE_SERVER_API_URL의 끝에 '/'가 있는지 확인하고 URL 결합 오류를 방지
    const baseURL = import.meta.env.VITE_SERVER_API_URL;
    // URL 끝이 '/'로 끝나면 빈 문자열, 아니면 '/'를 추가
    const separator = baseURL.endsWith('/') ? '' : '/';

    // window.location.href를 사용하여 백엔드 Google 인증 엔드포인트로 리다이렉트
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 3. API 호출 성공: 토큰 저장 및 Context 업데이트
  ...
  // ❌ [삭제] 불필요하고 에러를 유발하는 중복/빈 함수 정의 삭제됨
...
`}
        ...
        <button
          type="button"
          // ✅ [수정 2] 올바르게 정의된 handleGoogleLogin 함수와 연결
          onClick={handleGoogleLogin}
          // 구글 로그인은 로그인 폼의 isDisabled 상태를 따르지 않도록 disabled 속성 제거 (선택 사항)
          className={`
          w-full py-3 rounded-md text-lg font-bold transition-colors shadow-lg
${
  // 구글 버튼은 일반적으로 다른 스타일을 사용하므로 임의로 수정
  'bg-blue-600 text-white hover:bg-blue-700 cursor-pointer'
}
`}
        >
          <div className="flex items-center justify-center gap-4">
            {/* 🚨 [주의] 구글 로고 SVG/이미지 경로는 프로젝트에 맞게 확인해주세요. */}
            <img
