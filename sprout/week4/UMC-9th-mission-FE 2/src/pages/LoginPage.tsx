...
import type { ResponseSigninDto } from '../types/auth'; // ResponseSigninDto 추가
import { postSignin } from '../apis/auth'; // postSignin 함수 추가
...
const LoginPage = (): ReactElement => {
  ...
  // handleSubmit 함수: API 호출 로직 (이미지 요청 로직 그대로 복원)
  const handleSubmit = async () => {
    console.log(values);

    // 1. 유효성 검사 통과 여부 확인 (버튼 disabled 로직과 동일)
    if (isDisabled) {
      console.log('유효성 검사 실패 또는 빈 칸 존재. API 호출을 건너뜜');
      return;
    }

    // response 변수는 try 블록 외부에서 선언되어야 catch 블록 이후 console.log에서 접근 가능
    let response: ResponseSigninDto;

    try {
      // 2. 실제 API 연동 (이미지에서 요청한 핵심 로직)
      ...
      // 3. 에러 처리: alert(error.message) 대신 console.error 사용 (규칙 준수)
      console.error(
        error.message || '로그인 중 알 수 없는 오류가 발생했습니다.'
      );
      return; // 실패 시 이후 로직 실행 방지
    }

    // 4. console.log(response) 실행 (이미지 요청 로직 복원)
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center h-full bg-black text-white min-h-screen">
      ...
      <div className="flex flex-col items-center p-8 bg-gray-900 rounded-xl shadow-2xl w-full max-w-md mt-20">
        ...
        {/* 로그인 버튼: 빨간색 적용 */}
        <button
          type="button" // 폼 제출이 아닌 onClick 핸들러를 사용하므로 type="button" 유지
          onClick={handleSubmit} // 통합된 handleSubmit 호출
          disabled={isDisabled}
          className={`
