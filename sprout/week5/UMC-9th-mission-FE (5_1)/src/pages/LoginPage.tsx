...
const LoginPage = (): ReactElement => {
  ...
  // 💡 [수정] 이미 로그인되어 있다면, 원래 가려던 페이지 또는 '/my'로 이동합니다.
  useEffect(() => {
    if (accessToken) {
      ...
      const from =
        ...
        '/my';
      // 이 코드는 이미 로그인이 완료된 사용자를 보호 경로로 바로 보내줍니다.
  ...
  // 💡 [수정] handleSubmit 함수: 유효성 검사 및 API 성공 시에만 리다이렉트합니다.
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 3. API 호출 성공:
      // a) 토큰 저장 (Local Storage)
      setItem(response.data.accessToken);

      // b) Context 상태 업데이트 (Context가 로컬 스토리지의 토큰을 읽어가도록 트리거)
      await login(values);

      // c) 페이지 이동 (마이페이지로 이동)
      console.log(response);

      // ProtectedLayout이 보낸 원래 경로(from)를 사용하거나, /my로 직접 이동합니다.
      const from =
        (location.state as { from?: { pathname: string } })?.from?.pathname ||
        '/my';
