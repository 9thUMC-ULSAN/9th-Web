...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      // -> refresh 토큰도 만료되었으므로, 중복 재시도를 방지하기 위해 로그아웃 처리.
      ...
      if (!refreshPromise) {
        ...
        // (토큰 갱신 실패 시)
          ...
          removeAccessToken();
          removeRefreshToken();
          // (로그인 페이지로 리디렉션하는 로직이 추가될 수 있음)
        })
        .finally(() => {
          // 성공하든 실패하든, refreshPromise를 비워 다음 요청이 새 갱신을 시도하게 함
          refreshPromise = null;
        });
      } // (!refreshPromise) 블록 끝

      // 진행중인 refreshPromise가 해결될(resolve) 때까지 기다림
      return refreshPromise.then((newAccessToken: string) => {
        // 원본 요청의 Authorization 헤더를 갱신된 토큰으로 교체
